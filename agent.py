...
# --- Real fallback template: use pdfplumber to parse PDF into a DataFrame ---
...
def ensure_safe_parser(parser_path: Path, bank: str, pdf_path: Path):
    """
    Try to import and call parse(); if it doesn't return a DataFrame,
    rewrite with the pdfplumber fallback.
    """
    try:
...
def plan_node(state: dict) -> dict:
    ...
    # If CSV exists, give LLM a sample of expected columns to guide PDF parsing
    headers, sample_rows = "", ""
    if csv_path.exists():
...
Requirements:
* Use the pdfplumber library to read tables from the given PDF.
* Combine all tables into a single pandas DataFrame.
* Ensure the header row is taken from the first row of the first table.
* If possible, align the DataFrame columns to these CSV headers:
  {headers}
